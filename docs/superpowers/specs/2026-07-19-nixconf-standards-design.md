...
The design below was written for a plain `flake-utils` flake. During implementation
the flake framework was changed to **flake-parts** (at the maintainer's request).
The following differ from the sections below; where they conflict, these win:

- **Framework:** `flake-parts` (`flake-parts.lib.mkFlake`), not `flake-utils`. Inputs
  are `nixpkgs`, `flake-parts`, `treefmt-nix`, `git-hooks` — no `flake-utils`.
- **treefmt:** configured inline in `perSystem.treefmt` (no standalone `treefmt.nix`).
  treefmt-nix's `flakeModule` auto-provides `formatter` and `checks.treefmt`.
- **git-hooks:** wired via `git-hooks.flakeModule` (`perSystem.pre-commit.settings.hooks`),
  not `git-hooks.lib.run`. Adds `checks.pre-commit`; devShell uses
  `config.pre-commit.installationScript`.
- **Systems:** `["aarch64-darwin" "x86_64-linux"]` (dev machine + CI runner). The real
  aarch64-linux hosts will live in the top-level `flake` block later.
- **commitizen:** overridden with `doCheck = false` — its 4.13.9 test suite fails under
  Python 3.14 in current nixpkgs. Remove the override once nixpkgs is fixed.
- **branch-name hook:** adds `always_run = true` so it fires on every push.
- **devShell:** also includes `nil` (Nix LSP) and `git`.
- **.gitignore:** adds `.pre-commit-config.yaml` (generated by the devShell).
